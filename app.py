...
# Initialize the recommendation engine with data path and embeddings paths
processed_books_path = os.environ.get('PROCESSED_BOOKS_PATH', 'processed_data/processed_books.csv')
embeddings_path = os.environ.get('EMBEDDINGS_PATH', 'processed_data/embeddings_matrix.npy')
model_name_path = os.environ.get('MODEL_NAME_PATH', 'processed_data/model_name.txt')
...
def api_recommendations():
    ...
    try:
        ...
        # Set min_pages if length is "medium" or "long"
        if length_level and isinstance(length_level, str):
            length_level = length_level.lower()
            if length_level == "medium":
                min_pages = 300
            elif length_level == "long":
                min_pages = 500
