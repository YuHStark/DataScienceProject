...
class RecommendationEngine:
    def __init__(self, processed_data_path, embeddings_path=None, model_name_path=None):
        """Initialize the recommendation engine with processed book data and embeddings."""
        ...
        if embeddings_path and os.path.exists(embeddings_path):
            ...
            default_path = os.path.join(os.path.dirname(processed_data_path), "embeddings_matrix.npy")
            if os.path.exists(default_path):
                print(f"Loading embeddings from {default_path}")
                self.embeddings_matrix = np.load(default_path)
    ...
    def find_similar_books_knn(self, book_title, n=3):
        ...
        """
        if self.embeddings_matrix is None or self.knn_model is None:
            # If embeddings matrix or KNN model not available, return empty result
            print("Warning: Embeddings matrix or KNN model not initialized")
            return pd.DataFrame()
        ...
        distances, indices = self.knn_model.kneighbors(self.embeddings_matrix[book_idx].reshape(1, -1))
    ...
    def ensemble_recommendations(self, genre=None, style=None, min_rating=0, max_pages=None, min_pages=0, top_n=5):
        ...
        Args:
            ...
            min_pages (int): Minimum book pages
            top_n (int): Number of recommendations to return
...
if __name__ == "__main__":
    ...
    embeddings_path = "processed_data/embeddings_matrix.npy"
    model_name_path = "processed_data/model_name.txt"
